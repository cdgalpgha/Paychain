...
const ChainBadge = ({ chain }) => {
  ...
  return <span style={{ background: s.bg, color: s.color, padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 600, border: `1px solid ${s.border}`, letterSpacing: "0.02em", whiteSpace: "nowrap" }}>{chain}</span>;
};
...
const Particles = () => {
  const particles = Array.from({ length: 20 }, (_, i) => ({ id: i, size: Math.random() * 3 + 1, left: Math.random() * 100, top: Math.random() * 100, delay: Math.random() * 6, duration: Math.random() * 6 + 6, animNum: Math.floor(Math.random() * 3) + 1, color: ['#7c3aed','#2563eb','#06b6d4','#a78bfa','#60a5fa'][Math.floor(Math.random() * 5)], shape: Math.random() > 0.7 ? '0%' : '50%' }));
...
export default function App() {
  ...
  const brand = "#7c3aed", brandLight = "rgba(124,58,237,0.12)";
  ...
  const input = { width: "100%", padding: "12px 14px", border: `1px solid ${border2}`, borderRadius: 10, fontSize: 14, background: surface2, color: textPrimary, fontFamily: "inherit", boxSizing: "border-box", outline: "none", transition: "all 0.2s" };
  ...
  const modal = { position: "fixed", inset: 0, background: "rgba(3,1,10,0.88)", backdropFilter: "blur(12px)", display: "flex", alignItems: isMobile ? "flex-end" : "center", justifyContent: "center", zIndex: 100 };
  const modalBox = { background: `linear-gradient(135deg, ${surface} 0%, ${surface2} 100%)`, borderRadius: isMobile ? "20px 20px 0 0" : 20, padding: isMobile ? "24px 20px 40px" : 28, width: isMobile ? "100%" : 420, border: `1px solid ${border2}`, boxShadow: `0 0 60px rgba(124,58,237,0.2), 0 25px 60px rgba(0,0,0,0.7)`, maxHeight: isMobile ? "90vh" : "auto", overflowY: "auto" };
  const walletOptions = [
  ...
  const pageTitle = { wallet: "Treasury Wallet", employees: "Employees", payout: "Run Payroll", approvals: "Approvals", history: "Payroll History" }[page];

  // ── MOBILE ──────────────────────────────────────────────────────────────
  if (isMobile) {
    return (
      <div style={{ fontFamily: "'Outfit', system-ui, sans-serif", color: textPrimary, fontSize: 14, background: bg, minHeight: "100vh", position: "relative" }}>
        <Particles /><Confetti show={showConfetti} />
        <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh", position: "relative", zIndex: 1 }}>

          {/* Mobile Header */}
          <div style={{ padding: "12px 16px", background: surface, borderBottom: `1px solid ${border}`, display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 50, backdropFilter: "blur(10px)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <PayChainLogo size={30} />
              <div>
                <div style={{ fontSize: 14, fontWeight: 800, background: "linear-gradient(135deg, #a78bfa, #60a5fa)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>PayChain</div>
                <div style={{ fontSize: 9, color: textMuted, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.1em" }}>{pageTitle}</div>
              </div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              {txPending && <span style={{ fontSize: 10, color: amber, background: amberLight, padding: "3px 8px", borderRadius: 20, fontWeight: 600 }}>⏳</span>}
              {isConnected && isOwner && <span style={{ fontSize: 10, color: "#fbbf24", background: "rgba(251,191,36,0.1)", padding: "3px 8px", borderRadius: 20, fontWeight: 700 }}>👑</span>}
              <div onClick={isConnected ? () => disconnect() : connectWallet} style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 12px", border: isConnected ? `1px solid rgba(16,185,129,0.3)` : `1px solid ${border2}`, borderRadius: 20, cursor: "pointer", fontSize: 11, fontWeight: 600, background: isConnected ? "rgba(16,185,129,0.1)" : surface2, color: isConnected ? green : textSecondary }}>
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: isConnected ? green : border2, display: "inline-block" }} />
          ...
          {/* Mobile Content */}
          <div key={pageKey} style={{ flex: 1, overflowY: "auto", padding: "16px", paddingBottom: 88, background: `radial-gradient(ellipse 100% 40% at 50% 0%, rgba(124,58,237,0.06) 0%, transparent 60%), ${bg}` }}>

            {page === "wallet" && (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", paddingTop: 24, gap: 16 }}>
                <div style={{ position: "relative" }}>
                  <PayChainLogo size={80} />
                  <div style={{ position: "absolute", inset: -16, borderRadius: "50%", background: "radial-gradient(circle, rgba(124,58,237,0.15) 0%, transparent 70%)", animation: "logo-pulse 3s ease-in-out infinite", pointerEvents: "none" }} />
                </div>
                <div style={{ fontSize: 26, fontWeight: 900, textAlign: "center", background: "linear-gradient(135deg, #e2d9f3, #a78bfa, #60a5fa)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>On-Chain Payroll</div>
                <div style={{ fontSize: 14, color: textSecondary, textAlign: "center", lineHeight: 1.7, maxWidth: 280 }}>Connect your wallet to manage payroll on <span style={{ color: brand, fontWeight: 600 }}>Base Mainnet</span></div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, width: "100%" }}>
                  {walletOptions.map(w => (
                    <div key={w.name} onClick={connectWallet} style={{ ...glowCard, padding: "16px 14px", cursor: "pointer", display: "flex", alignItems: "center", gap: 12 }}>
                      <div style={{ width: 42, height: 42, borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", background: surface2, border: `1px solid ${border2}`, flexShrink: 0 }}>{w.logo}</div>
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 700, color: textPrimary }}>{w.name}</div>
                        <div style={{ fontSize: 10, color: textMuted }}>{w.chain}</div>
                      </div>
                      {isConnected && <div style={{ marginLeft: "auto", width: 8, height: 8, borderRadius: "50%", background: green, flexShrink: 0 }} />}
                    </div>
                  ))}
                </div>
                {isConnected && (
                  <div style={{ width: "100%", padding: "20px", border: `1px solid rgba(124,58,237,0.3)`, borderRadius: 20, background: `linear-gradient(135deg, rgba(124,58,237,0.1), rgba(37,99,235,0.05))` }}>
                    <div style={{ fontSize: 10, color: brand, marginBottom: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em" }}>✓ Connected</div>
                    <div style={{ fontFamily: "monospace", fontSize: 10, marginBottom: 14, color: textSecondary, wordBreak: "break-all", lineHeight: 1.6 }}>{address}</div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 16 }}>
                      {[["Network", "Base Mainnet", textPrimary], ["Treasury", `${parseFloat(treasuryBalance).toFixed(3)} ETH`, green], ["Role", isOwner ? "👑 Owner" : "Viewer", isOwner ? amber : textSecondary]].map(([label, val, color]) => (
                        <div key={label}><div style={{ fontSize: 9, color: textMuted, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 4 }}>{label}</div><div style={{ fontSize: 11, fontWeight: 700, color }}>{val}</div></div>
                      ))}
                    </div>
                    <div style={{ display: "flex", gap: 10 }}>
                      <button onClick={() => setShowDepositModal(true)} style={{ flex: 1, padding: "12px", border: `1px solid rgba(124,58,237,0.4)`, borderRadius: 12, cursor: "pointer", background: brandLight, color: "#a78bfa", fontSize: 13, fontFamily: "inherit", fontWeight: 700 }}>+ Deposit ETH</button>
                      <button onClick={() => disconnect()} style={{ flex: 1, padding: "12px", border: `1px solid rgba(239,68,68,0.3)`, borderRadius: 12, cursor: "pointer", background: redLight, color: red, fontSize: 13, fontFamily: "inherit", fontWeight: 700 }}>Disconnect</button>
                    </div>
                  </div>
                )}
              </div>
            ...
            {page === "employees" && (
              <div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
                  {[{ label: "On-Chain", val: contractEmployees.length, color: "#a78bfa", accent: brand }, { label: "Monthly Cost", val: `${parseFloat(payrollCost).toFixed(3)} ETH`, color: enoughFunds ? green : red, accent: enoughFunds ? green : red }].map(card => (
                    <div key={card.label} style={{ ...glowCard, padding: "14px 16px" }}>
                      ...
                      <div style={{ fontSize: 10, color: textMuted, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 6, fontWeight: 600 }}>{card.label}</div>
                      <div style={{ fontSize: 20, fontWeight: 800, color: card.color }}>{card.val}</div>
                    </div>
                  ))}
                </div>
                <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
                  <input style={{ ...input, flex: 1, padding: "10px 14px", fontSize: 13 }} placeholder="🔍 Search..." value={searchQ} onChange={e => setSearchQ(e.target.value)} />
                  <button style={{ ...btnBrandSm, padding: "10px 16px", whiteSpace: "nowrap" }} onClick={openAddModal}>+ Add</button>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {loading ? (
                    <div style={{ textAlign: "center", padding: "40px 0", color: textMuted }}><div style={{ width: 28, height: 28, border: `2px solid ${border2}`, borderTop: `2px solid ${brand}`, borderRadius: "50%", animation: "spin 0.8s linear infinite", margin: "0 auto 10px" }} />Loading...</div>
                  ) : filteredEmployees.length === 0 ? (
                    <div style={{ textAlign: "center", padding: "40px 0", color: textMuted }}><div style={{ fontSize: 32, marginBottom: 8 }}>👥</div>No employees yet.</div>
                  ) : filteredEmployees.map(e => (
                    <div key={e.id} style={{ ...glowCard, padding: "16px" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                        <div style={{ flex: 1, marginRight: 10 }}>
                          <div style={{ fontWeight: 700, color: textPrimary, fontSize: 15 }}>{e.name}{e.isOnChain && <span style={{ marginLeft: 8, fontSize: 10, color: cyan, background: "rgba(6,182,212,0.1)", padding: "2px 7px", borderRadius: 10, border: "1px solid rgba(6,182,212,0.2)" }}>⛓</span>}</div>
                          <div style={{ fontSize: 11, color: textMuted, fontFamily: "monospace", marginTop: 4 }}>{e.addr ? `${e.addr.slice(0,10)}...${e.addr.slice(-4)}` : "—"}</div>
                        </div>
                        <div style={{ fontSize: 15, fontWeight: 800, color: green, fontFamily: "monospace", whiteSpace: "nowrap" }}>{e.salary ? parseFloat(e.salary).toFixed(4) : "—"} <span style={{ fontSize: 10, color: textMuted }}>ETH</span></div>
                      </div>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                          <ChainBadge chain={e.chain} />
                          <span style={{ fontSize: 11, color: textMuted }}>{e.lastPaid || "—"}</span>
                        </div>
            ...
            {page === "payout" && (
              <div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
                  {[
                    { label: "Employees", val: contractEmployees.length, sub: "on-chain", color: "#a78bfa", accent: brand },
                    { label: "Payroll Cost", val: `${parseFloat(payrollCost).toFixed(4)} ETH`, sub: enoughFunds ? "✓ funded" : "⚠ low", color: enoughFunds ? green : red, accent: enoughFunds ? green : red },
                    { label: "Treasury", val: `${parseFloat(treasuryBalance).toFixed(4)} ETH`, sub: "balance", color: "#60a5fa", accent: blue },
                  ].map(card => (
                    <div key={card.label} style={{ ...glowCard, padding: "14px 16px" }}>
                      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, background: `linear-gradient(90deg, ${card.accent}, transparent)` }} />
                      <div style={{ fontSize: 9, color: textMuted, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 6, fontWeight: 600 }}>{card.label}</div>
                      <div style={{ fontSize: 14, fontWeight: 800, color: card.color, marginBottom: 2 }}>{card.val}</div>
                      <div style={{ fontSize: 10, color: textMuted }}>{card.sub}</div>
                    </div>
                  ))}
                </div>
                {!enoughFunds && parseFloat(payrollCost) > 0 && (
                  <div style={{ background: redLight, border: `1px solid rgba(239,68,68,0.25)`, borderRadius: 12, padding: "12px 14px", marginBottom: 14, fontSize: 13, color: red, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
                    <span>⚠️ Need <strong>{(parseFloat(payrollCost) - parseFloat(treasuryBalance)).toFixed(4)} more ETH</strong></span>
                    <button style={btnRedSm} onClick={() => setShowDepositModal(true)}>Deposit →</button>
                  </div>
                )}
                <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 16 }}>
                  {contractEmployees.map(e => (
                    <div key={e.id} style={{ ...glowCard, padding: "14px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div>
                        <div style={{ fontWeight: 700, color: textPrimary }}>{e.name}</div>
                        <div style={{ fontSize: 11, color: textMuted, fontFamily: "monospace", marginTop: 2 }}>{e.addr ? `${e.addr.slice(0,10)}...` : "—"}</div>
                      </div>
                      <div style={{ textAlign: "right" }}>
                        <div style={{ fontWeight: 700, color: green, fontSize: 14, fontFamily: "monospace" }}>{parseFloat(e.salary).toFixed(4)} ETH</div>
                        <div style={{ fontSize: 10, color: textMuted, marginTop: 2 }}>{e.lastPaid}</div>
                      </div>
                    </div>
                  ))}
                  {contractEmployees.length === 0 && <div style={{ textAlign: "center", padding: "30px", color: textMuted }}>No employees yet.</div>}
                </div>
                <div style={{ padding: "18px", borderRadius: 16, background: `linear-gradient(135deg, rgba(124,58,237,0.15), rgba(37,99,235,0.08))`, border: `1px solid rgba(124,58,237,0.25)` }}>
                  <div style={{ fontSize: 10, color: "#a78bfa", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 6 }}>Total to disburse</div>
                  <div style={{ fontSize: 28, fontWeight: 900, color: textPrimary, fontFamily: "monospace", marginBottom: 16 }}>{parseFloat(payrollCost).toFixed(4)} <span style={{ fontSize: 14, color: textSecondary }}>ETH</span></div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                    <button style={{ ...btn, textAlign: "center", padding: "13px" }} onClick={submitForApproval}>Submit for Approval</button>
                    <button style={{ ...btnBrand, textAlign: "center", padding: "14px", fontSize: 15, fontWeight: 800, opacity: (!isOwner || !enoughFunds || txPending) ? 0.5 : 1 }} onClick={executePayroll} disabled={!isOwner || !enoughFunds || txPending}>{txPending ? "⏳ Broadcasting..." : "🚀 Execute Payroll"}</button>
                  </div>
                </div>
              </div>
            )}
            ...
            {page === "approvals" && (
              <div>
                ...
                {approvals.length === 0 && <div style={{ textAlign: "center", padding: "60px 0", color: textMuted }}><div style={{ fontSize: 40, marginBottom: 12 }}>✅</div><div style={{ color: textSecondary, fontWeight: 600 }}>No pending approvals</div></div>}
                {approvals.map(a => (
                  <div key={a.id} style={{ ...glowCard, marginBottom: 12 }}>
                    <div style={{ padding: "14px 16px", background: `linear-gradient(90deg, ${surface2}, transparent)`, borderBottom: `1px solid ${border}` }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                        <div style={{ fontWeight: 700, color: textPrimary }}>{a.total.toFixed(4)} {a.token}</div>
                        <StatusBadge status={a.status} />
                      </div>
                      <div style={{ fontSize: 12, color: textMuted, marginBottom: a.status === "Pending" ? 10 : 0 }}>{a.date} · {a.count} employee(s)</div>
                      {a.status === "Pending" && <button style={{ ...btnBrandSm, width: "100%", textAlign: "center", padding: "10px" }} onClick={() => { setReviewingId(a.id); setShowReviewModal(true); }}>Review →</button>}
                    </div>
                  </div>
                ))}
              </div>
            ...
            {page === "history" && (
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                  <div>
                    <div style={{ fontSize: 12, color: textSecondary }}>{history.length} run(s)</div>
                    <div style={{ fontSize: 22, fontWeight: 800, color: green, fontFamily: "monospace" }}>{history.reduce((s, r) => s + r.total, 0).toFixed(4)} <span style={{ fontSize: 13, color: textMuted }}>ETH</span></div>
                  </div>
                ...
                {history.length === 0 && <div style={{ textAlign: "center", padding: "60px 0", color: textMuted }}><div style={{ fontSize: 40, marginBottom: 12 }}>📋</div><div style={{ color: textSecondary, fontWeight: 600 }}>No payroll runs yet</div></div>}
                {history.map(r => (
                  <div key={r.id} style={{ ...glowCard, marginBottom: 12 }}>
                    <div onClick={() => setExpandedRun(expandedRun === r.id ? null : r.id)} style={{ padding: "14px 16px", cursor: "pointer" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                        <div style={{ fontWeight: 700, color: textPrimary }}>Run #{r.id} — <span style={{ color: green, fontFamily: "monospace" }}>{r.total.toFixed(4)} {r.token}</span></div>
                        <span style={{ color: textMuted, fontSize: 14 }}>{expandedRun === r.id ? "▲" : "▼"}</span>
                      </div>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <div style={{ fontSize: 12, color: textMuted }}>{r.date} · {r.count} employee(s)</div>
                    ...
                    {expandedRun === r.id && (
                      <div style={{ padding: "0 16px 14px" }}>
                        {r.items.map((i, idx) => (
                          <div key={idx} style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderTop: `1px solid ${border}` }}>
                            <div>
                              <div style={{ fontWeight: 600, color: textPrimary, fontSize: 13 }}>{i.name}</div>
                              <div style={{ fontSize: 10, color: textMuted, fontFamily: "monospace" }}>{i.addr.slice(0,12)}...</div>
                            </div>
                            <div style={{ color: green, fontWeight: 700, fontFamily: "monospace", fontSize: 13 }}>{i.amt.toFixed(4)} {i.token}</div>
                          </div>
                        ))}
                      </div>
                    )}
          ...
          {/* Mobile Bottom Nav */}
          <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: `${surface}f0`, backdropFilter: "blur(20px)", borderTop: `1px solid ${border}`, display: "flex", zIndex: 50 }}>
            {navItems.map(item => (
              <button key={item.id} onClick={() => navigateTo(item.id)} style={{ flex: 1, padding: "8px 4px 14px", border: "none", background: "transparent", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, position: "relative", fontFamily: "inherit" }}>
                <div style={{ width: 32, height: 32, borderRadius: 10, background: page === item.id ? `linear-gradient(135deg, ${brand}, ${blue})` : "transparent", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, color: page === item.id ? "#fff" : textMuted, transition: "all 0.2s", boxShadow: page === item.id ? `0 0 12px rgba(124,58,237,0.5)` : "none" }}>{item.icon}</div>
                <span style={{ fontSize: 9, fontWeight: page === item.id ? 700 : 400, color: page === item.id ? "#a78bfa" : textMuted }}>{item.label}</span>
        ...
        {/* MODALS */}
        {showDepositModal && <div style={modal}><div style={modalBox}><div style={{ fontSize: 18, fontWeight: 800, color: textPrimary, marginBottom: 4 }}>💰 Deposit ETH</div><div style={{ fontSize: 13, color: textSecondary, marginBottom: 20 }}>Treasury: <span style={{ color: green, fontWeight: 700 }}>{parseFloat(treasuryBalance).toFixed(4)} ETH</span></div><label style={{ display: "block", fontSize: 11, color: textMuted, marginBottom: 6, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em" }}>Amount (ETH)</label><input type="number" step="0.001" placeholder="e.g. 0.05" value={depositAmount} onChange={e => setDepositAmount(e.target.value)} style={input} /><div style={{ display: "flex", gap: 10, marginTop: 24 }}><button style={{ ...btn, flex: 1 }} onClick={() => setShowDepositModal(false)}>Cancel</button><button style={{ ...btnBrand, flex: 1 }} onClick={depositToTreasury} disabled={txPending}>{txPending ? "⏳" : "Deposit"}</button></div></div></div>}
        {showAddModal && <div style={modal}><div style={modalBox}><div style={{ fontSize: 18, fontWeight: 800, color: textPrimary, marginBottom: 4 }}>{editingEmployee ? "✏️ Edit Employee" : "👤 Add Employee"}</div><div style={{ fontSize: 12, color: textSecondary, marginBottom: 20, padding: "8px 12px", background: brandLight, borderRadius: 8, border: `1px solid rgba(124,58,237,0.2)` }}>⛓ Transaction on Base Mainnet</div>{[["Full name", "name", "text", "Sarah Chen"], ["Email (optional)", "email", "email", "sarah@company.com"], ["Wallet address", "addr", "text", "0x..."], ["Monthly salary (ETH)", "salary", "number", "0.05"]].map(([label, field, type, ph]) => (<div key={field} style={{ marginBottom: 14 }}><label style={{ display: "block", fontSize: 11, color: textMuted, marginBottom: 6, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em" }}>{label}</label><input type={type} placeholder={ph} value={formData[field]} onChange={e => setFormData({ ...formData, [field]: e.target.value })} style={input} /></div>))}<div style={{ display: "flex", gap: 10, marginTop: 8 }}><button style={{ ...btn, flex: 1 }} onClick={() => setShowAddModal(false)}>Cancel</button><button style={{ ...btnBrand, flex: 1 }} onClick={saveEmployee} disabled={txPending}>{txPending ? "⏳" : "Add"}</button></div></div></div>}
        {showCsvModal && <div style={modal}><div style={modalBox}><div style={{ fontSize: 18, fontWeight: 800, color: textPrimary, marginBottom: 8 }}>📂 CSV Import</div><textarea rows={5} placeholder="name,email,wallet,chain,salary" value={csvText} onChange={e => setCsvText(e.target.value)} style={{ ...input, resize: "vertical" }} /><div style={{ display: "flex", gap: 10, marginTop: 16 }}><button style={{ ...btn, flex: 1 }} onClick={() => setShowCsvModal(false)}>Cancel</button><button style={{ ...btnBrand, flex: 1 }} onClick={importCsv}>Import</button></div></div></div>}
        {showSignerModal && <div style={modal}><div style={modalBox}><div style={{ fontSize: 18, fontWeight: 800, color: textPrimary, marginBottom: 8 }}>⚙️ Signers</div>{signers.map(s => (<div key={s.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: `1px solid ${border}` }}><div style={{ width: 32, height: 32, borderRadius: "50%", background: brandLight, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, color: "#a78bfa", flexShrink: 0 }}>{s.name.split(" ").map(x => x[0]).join("").slice(0, 2)}</div><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13, fontWeight: 600, color: textPrimary, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.name}</div></div><button style={btnRedSm} onClick={() => setSigners(signers.filter(x => x.id !== s.id))}>✕</button></div>))}<div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 14 }}><input placeholder="Name" value={newSignerName} onChange={e => setNewSignerName(e.target.value)} style={input} /><input placeholder="0x..." value={newSignerAddr} onChange={e => setNewSignerAddr(e.target.value)} style={input} /><button style={btnBrandSm} onClick={() => { if (!newSignerName || !newSignerAddr) return; setSigners([...signers, { id: nextSignerId, name: newSignerName, addr: newSignerAddr }]); setNextSignerId(nextSignerId + 1); setNewSignerName(""); setNewSignerAddr(""); }}>Add Signer</button></div><button style={{ ...btnBrand, width: "100%", marginTop: 16, textAlign: "center", padding: "12px" }} onClick={() => setShowSignerModal(false)}>Done</button></div></div>}
        {showReviewModal && (() => { const a = approvals.find(x => x.id === reviewingId); if (!a) return null; return <div style={modal}><div style={modalBox}><div style={{ fontSize: 18, fontWeight: 800, color: textPrimary, marginBottom: 20 }}>🔍 Review Payroll</div>{[["Date", `${a.date} ${a.time}`], ["Employees", a.count], ["Token", a.token], ["Total", `${a.total.toFixed(4)} ETH`]].map(([label, val]) => <div key={label} style={{ display: "flex", justifyContent: "space-between", fontSize: 14, padding: "10px 0", borderBottom: `1px solid ${border}` }}><span style={{ color: textMuted, fontWeight: 600 }}>{label}</span><span style={{ fontWeight: label === "Total" ? 800 : 600, color: label === "Total" ? green : textPrimary }}>{val}</span></div>)}<div style={{ display: "flex", gap: 10, marginTop: 24 }}><button style={{ ...btnRed, flex: 1, textAlign: "center" }} onClick={rejectApproval}>Reject</button><button style={{ ...btnGreen, flex: 1, textAlign: "center" }} onClick={approveAndExecute} disabled={txPending}>{txPending ? "⏳" : "✓ Approve"}</button></div></div></div>; })()}
        {success && <div style={{ ...modal, backdropFilter: "blur(16px)" }}><div style={{ ...modalBox, textAlign: "center", margin: "0 16px", borderRadius: 20 }}><div style={{ width: 64, height: 64, borderRadius: "50%", background: success.bg, border: `1px solid ${success.color}33`, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px", fontSize: 28, boxShadow: `0 0 40px ${success.color}44` }}>{success.icon}</div><div style={{ fontSize: 18, fontWeight: 800, marginBottom: 8, color: success.color }}>{success.title}</div><div style={{ fontSize: 14, color: textSecondary, lineHeight: 1.7 }}>{success.msg}</div><button style={{ ...btnBrand, marginTop: 24, width: "100%", padding: "14px", fontSize: 15, fontWeight: 800 }} onClick={() => setSuccess(null)}>Done ✓</button></div></div>}
      </div>
    );
  }
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0, position: "relative", zIndex: 1 }}>
          <div style={{ padding: "16px 28px", borderBottom: `1px solid ${border}`, display: "flex", alignItems: "center", justifyContent: "space-between", background: `linear-gradient(90deg, ${surface} 0%, rgba(13,9,32,0.8) 100%)`, backdropFilter: "blur(10px)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <span style={{ fontSize: 17, fontWeight: 800, letterSpacing: "-0.02em", background: "linear-gradient(135deg, #e2d9f3, #a78bfa)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>{pageTitle}</span>
          ...
          <div key={pageKey} className="page-transition" style={{ flex: 1, overflowY: "auto", padding: 28, background: `radial-gradient(ellipse 80% 60% at 50% -20%, rgba(124,58,237,0.06) 0%, transparent 60%), ${bg}` }}>
            {page === "wallet" && (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", gap: 10 }}>
                <div style={{ position: "relative", marginBottom: 20 }}><PayChainLogo size={100} /><div style={{ position: "absolute", inset: -20, borderRadius: "50%", background: "radial-gradient(circle, rgba(124,58,237,0.15) 0%, transparent 70%)", animation: "logo-pulse 3s ease-in-out infinite", pointerEvents: "none" }} /></div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, width: 440 }}>
                  {walletOptions.map(w => (<div key={w.name} onClick={connectWallet} className="card-hover" style={{ ...glowCard, padding: "22px 20px", cursor: "pointer", display: "flex", alignItems: "center", gap: 16 }}><div style={{ width: 54, height: 54, borderRadius: 14, display: "flex", alignItems: "center", justifyContent: "center", background: surface2, border: `1px solid ${border2}`, flexShrink: 0, overflow: "hidden" }}>{w.logo}</div><div><div style={{ fontSize: 15, fontWeight: 700, color: textPrimary }}>{w.name}</div><div style={{ fontSize: 11, color: textMuted, marginTop: 3 }}>{w.chain}</div></div>{isConnected && <div style={{ marginLeft: "auto", width: 9, height: 9, borderRadius: "50%", background: green, boxShadow: `0 0 10px ${green}` }} />}</div>))}
                </div>
                {isConnected && (
                  <div style={{ marginTop: 24, padding: "24px", border: `1px solid rgba(124,58,237,0.3)`, borderRadius: 20, width: 440, background: `linear-gradient(135deg, rgba(124,58,237,0.1), rgba(37,99,235,0.05))`, boxShadow: "0 0 40px rgba(124,58,237,0.15)" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 20 }}>
                      {[["Network", "Base Mainnet", textPrimary], ["Treasury", `${parseFloat(treasuryBalance).toFixed(4)} ETH`, green], ["Role", isOwner ? "👑 Owner" : "Viewer", isOwner ? amber : textSecondary]].map(([label, val, color]) => (<div key={label}><div style={{ fontSize: 9, color: textMuted, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 4 }}>{label}</div><div style={{ fontSize: 13, fontWeight: 700, color }}>{val}</div></div>))}
                    </div>
                    <div style={{ display: "flex", gap: 10 }}>
            ...
            {page === "employees" && (
              <div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
                  <div style={{ position: "relative" }}><span style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: textMuted, fontSize: 14 }}>🔍</span><input style={{ ...input, width: 200, paddingLeft: 34 }} placeholder="Search employees..." value={searchQ} onChange={e => setSearchQ(e.target.value)} /></div>
                  {["All", "Base Mainnet", ...CHAINS.slice(0, 2)].map(c => <button key={c} onClick={() => setChainFilter(c)} style={{ ...btnSm, borderRadius: 20, background: chainFilter === c ? `linear-gradient(135deg, ${brand}, ${blue})` : surface2, color: chainFilter === c ? "#fff" : textSecondary, border: chainFilter === c ? "none" : `1px solid ${border}` }}>{c}</button>)}
                  <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}><button style={btnSm} onClick={() => setShowCsvModal(true)}>📂 CSV</button><button className="btn-hover-glow" style={btnBrandSm} onClick={openAddModal}>+ Add Employee</button></div>
                </div>
                {loading ? <div style={{ textAlign: "center", padding: "60px 0", color: textMuted }}><div style={{ width: 32, height: 32, border: `2px solid ${border2}`, borderTop: `2px solid ${brand}`, borderRadius: "50%", animation: "spin 0.8s linear infinite", margin: "0 auto 12px" }} />Loading...</div> : (
                  <div style={{ ...glowCard, overflow: "hidden" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                      ...
                      <tbody>
                        {filteredEmployees.map((e, idx) => (<tr key={e.id} style={{ borderBottom: idx < filteredEmployees.length - 1 ? `1px solid ${border}` : "none", transition: "background 0.15s" }} onMouseEnter={el => el.currentTarget.style.background = `rgba(124,58,237,0.05)`} onMouseLeave={el => el.currentTarget.style.background = "transparent"}><td style={{ padding: "14px 16px", fontWeight: 700, color: textPrimary }}>{e.name}{e.isOnChain && <span style={{ marginLeft: 8, fontSize: 10, color: cyan, background: "rgba(6,182,212,0.1)", padding: "2px 7px", borderRadius: 10, border: "1px solid rgba(6,182,212,0.2)", fontWeight: 600 }}>⛓</span>}</td><td style={{ padding: "14px 16px", fontFamily: "'Space Mono', monospace", fontSize: 10, color: textSecondary }}>{e.addr ? `${e.addr.slice(0,8)}...${e.addr.slice(-6)}` : "—"}</td><td style={{ padding: "14px 16px" }}><ChainBadge chain={e.chain} /></td><td style={{ padding: "14px 16px", fontWeight: 700, color: green, fontFamily: "'Space Mono', monospace", fontSize: 13 }}>{e.salary ? `${parseFloat(e.salary).toFixed(4)}` : <span style={{ color: textMuted }}>—</span>} <span style={{ color: textMuted, fontSize: 11 }}>ETH</span></td><td style={{ padding: "14px 16px", fontSize: 12, color: textMuted }}>{e.lastPaid || "—"}</td><td style={{ padding: "14px 16px" }}><div style={{ display: "flex", gap: 6 }}><button style={btnSm} onClick={() => openEditModal(e)}>Edit</button>{e.isOnChain && isOwner && <button style={btnRedSm} onClick={() => removeEmployee(e.contractId, e.name)} disabled={txPending}>Remove</button>}</div></td></tr>))}
            ...
            {page === "payout" && (
              <div>
                ...
                {!enoughFunds && parseFloat(payrollCost) > 0 && <div style={{ background: redLight, border: `1px solid rgba(239,68,68,0.25)`, borderRadius: 14, padding: "14px 18px", marginBottom: 20, fontSize: 13, color: red, display: "flex", alignItems: "center", justifyContent: "space-between" }}><span>⚠️ Need <strong>{(parseFloat(payrollCost) - parseFloat(treasuryBalance)).toFixed(4)} more ETH</strong></span><button style={{ ...btnSm, background: "rgba(239,68,68,0.15)", color: red, border: `1px solid rgba(239,68,68,0.3)` }} onClick={() => setShowDepositModal(true)}>Deposit ETH →</button></div>}
                <div style={{ ...glowCard, overflow: "hidden", marginBottom: 20 }}>
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    ...
                    <tbody>{contractEmployees.map((e, idx) => <tr key={e.id} style={{ borderBottom: idx < contractEmployees.length - 1 ? `1px solid ${border}` : "none" }}><td style={{ padding: "14px 16px", fontWeight: 700, color: textPrimary }}>{e.name}</td><td style={{ padding: "14px 16px", fontFamily: "'Space Mono', monospace", fontSize: 10, color: textSecondary }}>{e.addr ? `${e.addr.slice(0,8)}...${e.addr.slice(-6)}` : "—"}</td><td style={{ padding: "14px 16px" }}><ChainBadge chain={e.chain} /></td><td style={{ padding: "14px 16px", fontWeight: 700, color: green, fontFamily: "'Space Mono', monospace" }}>{parseFloat(e.salary).toFixed(4)} ETH</td><td style={{ padding: "14px 16px", fontSize: 12, color: textMuted }}>{e.lastPaid}</td></tr>)}{contractEmployees.length === 0 && <tr><td colSpan={5} style={{ padding: "40px", textAlign: "center", color: textMuted }}>No on-chain employees yet.</td></tr>}</tbody>
                  </table>
                </div>
                <div style={{ padding: "20px 24px", borderRadius: 18, display: "flex", alignItems: "center", justifyContent: "space-between", background: `linear-gradient(135deg, rgba(124,58,237,0.15), rgba(37,99,235,0.08))`, border: `1px solid rgba(124,58,237,0.25)` }}>
                  ...
                  <div style={{ display: "flex", gap: 12 }}><button style={btn} onClick={submitForApproval}>Submit for approval</button><button className="btn-hover-glow" style={{ ...btnBrand, padding: "12px 28px", fontSize: 14, fontWeight: 800, opacity: (!isOwner || !enoughFunds || txPending) ? 0.5 : 1 }} onClick={executePayroll} disabled={!isOwner || !enoughFunds || txPending}>{txPending ? "⏳ Broadcasting..." : "🚀 Execute Payroll"}</button></div>
                </div>
              </div>
            )}
            {page === "approvals" && (
              <div>
                ...
                {approvals.length === 0 && <div style={{ textAlign: "center", padding: "80px 0", color: textMuted }}><div style={{ fontSize: 48, marginBottom: 16 }}>✅</div><div style={{ fontSize: 15, fontWeight: 600, color: textSecondary }}>No pending approvals</div></div>}
                {approvals.map(a => (<div key={a.id} className="card-hover" style={{ ...glowCard, marginBottom: 14, overflow: "hidden" }}><div style={{ padding: "16px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", background: `linear-gradient(90deg, ${surface2}, transparent)`, borderBottom: `1px solid ${border}` }}><div><div style={{ fontWeight: 700, fontSize: 15, color: textPrimary }}>{a.total.toFixed(4)} {a.token} — {a.count} employee(s)</div><div style={{ fontSize: 12, color: textMuted, marginTop: 4 }}>{a.date} at {a.time}</div></div><div style={{ display: "flex", alignItems: "center", gap: 10 }}><StatusBadge status={a.status} />{a.status === "Pending" && <button className="btn-hover-glow" style={btnBrandSm} onClick={() => { setReviewingId(a.id); setShowReviewModal(true); }}>Review →</button>}</div></div><div style={{ padding: "14px 20px" }}>{a.signerStatuses.map(s => <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: `1px solid ${border}` }}><div style={{ width: 34, height: 34, borderRadius: "50%", background: brandLight, border: `1px solid rgba(124,58,237,0.25)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 800, color: "#a78bfa" }}>{s.name.split(" ").map(x => x[0]).join("").slice(0, 2)}</div><div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 600, color: textPrimary }}>{s.name}</div><div style={{ fontFamily: "'Space Mono', monospace", fontSize: 10, color: textMuted }}>{s.addr}</div></div><StatusBadge status={s.status} /></div>)}</div></div>))}
              </div>
            )}
            {page === "history" && (
              <div>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
                  <div><div style={{ fontSize: 13, color: textSecondary }}>{history.length} payroll run(s)</div><div style={{ fontSize: 22, fontWeight: 800, color: green, fontFamily: "'Space Mono', monospace" }}>{history.reduce((s, r) => s + r.total, 0).toFixed(4)} <span style={{ fontSize: 14, color: textMuted }}>ETH total</span></div></div>
                ...
                {history.length === 0 && <div style={{ textAlign: "center", padding: "80px 0", color: textMuted }}><div style={{ fontSize: 48, marginBottom: 16 }}>📋</div><div style={{ fontSize: 15, fontWeight: 600, color: textSecondary }}>No payroll runs yet</div></div>}
                {history.map(r => (<div key={r.id} className="card-hover" style={{ ...glowCard, marginBottom: 14, overflow: "hidden" }}><div onClick={() => setExpandedRun(expandedRun === r.id ? null : r.id)} style={{ padding: "16px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", background: `linear-gradient(90deg, ${surface2}, transparent)`, cursor: "pointer" }}><div><div style={{ fontWeight: 700, fontSize: 15, color: textPrimary }}>Run #{r.id} — <span style={{ color: green, fontFamily: "'Space Mono', monospace" }}>{r.total.toFixed(4)} {r.token}</span></div><div style={{ fontSize: 12, color: textMuted, marginTop: 4 }}>{r.date} at {r.time} · {r.count} employee(s)</div></div><div style={{ display: "flex", alignItems: "center", gap: 10 }}><StatusBadge status={r.status} /><span style={{ color: textMuted, fontSize: 16 }}>{expandedRun === r.id ? "▲" : "▼"}</span></div></div>{expandedRun === r.id && <div style={{ padding: "0 20px 16px" }}><table style={{ width: "100%", borderCollapse: "collapse", marginTop: 14 }}><thead><tr>{["Employee", "Wallet", "Chain", "Amount"].map(h => <th key={h} style={{ fontSize: 10, color: textMuted, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", padding: "10px 12px", borderBottom: `1px solid ${border}`, textAlign: "left" }}>{h}</th>)}</tr></thead><tbody>{r.items.map((i, idx) => <tr key={idx}><td style={{ padding: "12px", fontWeight: 600, color: textPrimary }}>{i.name}</td><td style={{ padding: "12px", fontFamily: "'Space Mono', monospace", fontSize: 10, color: textSecondary }}>{i.addr}</td><td style={{ padding: "12px" }}><ChainBadge chain={i.chain} /></td><td style={{ padding: "12px", color: green, fontWeight: 700, fontFamily: "'Space Mono', monospace" }}>{i.amt.toFixed(4)} {i.token}</td></tr>)}</tbody></table></div>}</div>))}
              </div>
            )}
          </div>
        </div>
