...
const ChainBadge = ({ chain }) => {
  ...
  return (
    <span style={{ background: s.bg, color: s.color, padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 600, border: `1px solid ${s.border}`, letterSpacing: "0.02em" }}>
      {chain}
    </span>
  );
};
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span style={{ background: s.bg, color: s.color, padding: "3px 12px", borderRadius: 20, fontSize: 11, fontWeight: 700, border: `1px solid ${s.border}`, letterSpacing: "0.05em", textTransform: "uppercase" }}>
      {status}
    </span>
  );
};
...
const Particles = () => {
  const particles = Array.from({ length: 30 }, (_, i) => ({
    ...
    color: ['#7c3aed', '#2563eb', '#06b6d4', '#a78bfa', '#60a5fa'][Math.floor(Math.random() * 5)],
    shape: Math.random() > 0.7 ? '0%' : '50%',
  }));
  return (
...
const Confetti = ({ show }) => {
  ...
  const pieces = Array.from({ length: 50 }, (_, i) => ({
    ...
    color: ['#7c3aed', '#2563eb', '#06b6d4', '#a78bfa', '#60a5fa', '#f59e0b', '#10b981'][Math.floor(Math.random() * 7)],
    animNum: Math.floor(Math.random() * 6) + 1, shape: Math.random() > 0.5 ? '50%' : '2px',
  }));
  return (
...
export default function App() {
  ...
  useEffect(() => {
    if (activeEmployeesData) {
      ...
      setContractEmployees(ids.map((id, i) => ({
  ...
  function exportCSV() {
    ...
    a.download = "payroll-report.csv"; a.click();
  }
          ...
          <div style={{ padding: "16px 28px", borderBottom: `1px solid ${border}`, display: "flex", alignItems: "center", justifyContent: "space-between", background: `linear-gradient(90deg, ${surface} 0%, rgba(13,9,32,0.8) 100%)`, backdropFilter: "blur(10px)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <span style={{ fontSize: 17, fontWeight: 800, letterSpacing: "-0.02em", background: "linear-gradient(135deg, #e2d9f3, #a78bfa)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
                {{ wallet: "Treasury Wallet", employees: "Employees", payout: "Run Payroll", approvals: "Approvals", history: "Payroll History" }[page]}
              </span>
                  ...
                  {[{ name: "MetaMask", icon: "🦊", chain: "EVM chains" }, { name: "Rabby", icon: "🐰", chain: "EVM chains" }, { name: "Phantom", icon: "👻", chain: "Solana + EVM" }, { name: "WalletConnect", icon: "🔗", chain: "Any wallet" }].map(w => (
                    <div key={w.name} onClick={() => connectWallet()} className="card-hover"
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 11, color: textMuted, marginTop: 2 }}>{w.chain}</div>
                      </div>
                      {isConnected && <div style={{ marginLeft: "auto", width: 8, height: 8, borderRadius: "50%", background: green, boxShadow: `0 0 8px ${green}` }} />}
                    </div>
                  ))}
                </div>
                {isConnected && (
                  <div style={{ marginTop: 24, padding: "24px", border: `1px solid rgba(124,58,237,0.3)`, borderRadius: 20, width: 420, background: `linear-gradient(135deg, rgba(124,58,237,0.1), rgba(37,99,235,0.05))`, boxShadow: "0 0 40px rgba(124,58,237,0.15)" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 20 }}>
                      {[["Network", "Base Sepolia", textPrimary], ["Treasury", `${parseFloat(treasuryBalance).toFixed(4)} ETH`, green], ["Role", isOwner ? "👑 Owner" : "Viewer", isOwner ? amber : textSecondary]].map(([label, val, color]) => (
                        <div key={label}>
                          <div style={{ fontSize: 9, color: textMuted, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 4 }}>{label}</div>
              ...
              <div>
                    ...
                    <tbody>
                      {contractEmployees.map((e, idx) => (
                        <tr key={e.id} style={{ borderBottom: idx < contractEmployees.length - 1 ? `1px solid ${border}` : "none" }}>
                          <td style={{ padding: "14px 16px", fontWeight: 700, color: textPrimary }}>{e.name}</td>
                          <td style={{ padding: "14px 16px", fontFamily: "'Space Mono', monospace", fontSize: 10, color: textSecondary }}>{e.addr ? `${e.addr.slice(0,8)}...${e.addr.slice(-6)}` : "—"}</td>
                          <td style={{ padding: "14px 16px" }}><ChainBadge chain={e.chain} /></td>
                          <td style={{ padding: "14px 16px", fontWeight: 700, color: green, fontFamily: "'Space Mono', monospace" }}>{parseFloat(e.salary).toFixed(4)} ETH</td>
                          <td style={{ padding: "14px 16px", fontSize: 12, color: textMuted }}>{e.lastPaid}</td>
                        </tr>
                      ))}
                ...
                <div style={{ padding: "20px 24px", borderRadius: 18, display: "flex", alignItems: "center", justifyContent: "space-between", background: `linear-gradient(135deg, rgba(124,58,237,0.15), rgba(37,99,235,0.08))`, border: `1px solid rgba(124,58,237,0.25)`, boxShadow: "0 0 30px rgba(124,58,237,0.1)" }}>
                  <div>
                    ...
                    <div style={{ fontSize: 30, fontWeight: 900, color: textPrimary, letterSpacing: "-0.03em", fontFamily: "'Space Mono', monospace" }}>{parseFloat(payrollCost).toFixed(4)} <span style={{ fontSize: 16, color: textSecondary }}>ETH</span></div>
                  </div>
